...
{
    public partial class sonay : Form
    {
        public sonay()
        {
            InitializeComponent();
        ...
        void griddoldur3()
        {
            da = new SqlDataAdapter("Select *From Tbl_sonay", conn);
            ds = new DataSet();
            conn.Open();
            da.Fill(ds, "Tbl_sonay");
            dataGridView1.DataSource = ds.Tables["Tbl_sonay"];
            conn.Close();
        }
        private void button1_Click(object sender, EventArgs e)
        {
            conn.Open();
            SqlCommand cmd = new SqlCommand("insert into tbl_sonay(Tc,soru1,soru2,soru3,soru4,soru5,ekstra)values(@s1,@s2,@s3,@s4,@s5,@s6,@s7)", conn);
        ...
        private void sonay_Load(object sender, EventArgs e)
        {
            griddoldur3();
        }
        ...
        {
            conn.Open();
            SqlCommand cmd = new SqlCommand("delete from Tbl_sonay where Tc=@t1", conn);
